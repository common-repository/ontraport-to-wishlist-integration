...
class ontraportWishlistHelper {
	...
	function add_login_tag($opid)
	{
		...
		$this->request('cdata','add_tag', 
			"<contact id='{$opid}'>
				<tag>".$this->get_prefix()." > ".$domain_name." - Active</tag>
			</contact>"
		);
	...
	function update_last_login_date($opid,$oap_section)
	{
		$this->request('cdata','update', 
			"<contact id='{$opid}'>
				...
				<field name='".$this->get_prefix()." Last Login Date'>".time()."</field>
				</Group_Tag>
			</contact>"
		);
	...
	function create_wl_login_date_field($oap_section)
	{
		...
		$mresult=$this->request('cdata','edit_section', 
			'<data>
			<Group_Tag name="'.$oap_section.'">
			<field name="'.$this->get_prefix().' Last Login Date" type="fulldate"/>
			</Group_Tag>
			</data>'
		);
	...
	public function request($req, $type, $data) {  
		...
		$request = "https://api.ontraport.com/{$req}.php";
	...
	function oa_wl_calls() {
		...
		if(isset($_GET['oa_wl_call'])) {
			...
			if($action == 'addlevel') {  //email, pass, fname, lname, level
				...
				if(!$userid){
					$password = (!empty($source['pass']) && strlen($source['pass']) >= 10) ? $source['pass'] : wp_generate_password(10, false);
			...
			if($action == 'wlimport') {
				...
				foreach($wlmLevels as $k => $mem) {
					if(stripos($tags,$this->get_prefix().' > '.$mem['name']) !== false && !(stripos($tags,($this->get_prefix().' > '.$mem['name'] . "PAYF")) !== false))
						$levs[] = $k;
				}
				...
				//file_put_contents("wl.txt", json_encode($source).json_encode($_POST)."a".$tags."a".json_encode($tags).json_encode($levs)."userid=".$userid."pass=".$correctedpass);
			...
			if($action == 'wlremove') {
				...
				$tags = $this->get_tags($_POST['tags']);//$_POST['tags'];
				$email = $_GET['email'];
			...
			if($action == 'payfcheck') {
				...
				//print_r($wlmLevels);
				foreach($wlmLevels as $k => $mem) {
					if(stripos($tags,($this->get_prefix().' > '.$mem['name']." PAYF")) !== false)
						$levs[] = $mem['ID'];
					//$init++;
					//echo $mem['ID'].'<br>';
				}
	...
	function get_tags($tag_ids){
		if(strpos($tag_ids, '*/*')!==false){
			...
			foreach($tag_ids as $tag_id){
				$tag= json_decode(file_get_contents('https://api.ontraport.com/1/object?objectID=14&id='.$tag_id, false, $context));
				if(isset($tag->data->tag_name)){
					$tags.=$tag->data->tag_name.', ';
				}
			}
			return $tags;
		}
	...
	function get_prefix(){
		$field_arr=get_option('wlhelper_oap_wlpwd');
		$oap_field=explode( "_", $field_arr['text_string']);
		return $oap_field[0];
	}
}
